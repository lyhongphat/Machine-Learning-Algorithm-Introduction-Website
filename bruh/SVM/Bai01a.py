...
def executeThisFunction():
    ...
    code = '''vi_tri_min = np.argmin(distance)
    min_val = np.min(distance)'''
    st.code(code, language="python")
    st.write("Tìm được vị trí của điểm xa nhất và gàn nhất của điểm đối với boundary")
    ...
    for i in range(0, SIZE):
        if (distance[i] - min_val) <= 1.0E-3:
            ...
            vi_tri.append(i)'''
    st.code(code, language="python")
    st.write("Sử dụng mang vi_tri để lưu tất cả các điểm cách điểm gần đường boundary nhất một khoảng nhỏ hơn 1.0E-3")
